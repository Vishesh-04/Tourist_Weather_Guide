...
def generate_response(user_input):
    ...
    async def handle_query_response(ctx: Context, _, msg: QueryTemperatureResponse):
        ...
        if msg.status == TemperatureStatus.COLD:
            ctx.logger.warning(
                f"{Fore.BLUE}{msg.status.value} at {Fore.BLUE}{msg.time}, Consider taking some winter essentials with you!! Current Temperature is {Fore.BLUE}{temperature}°C{Style.RESET_ALL}.")

        elif msg.status == TemperatureStatus.HOT:
            ctx.logger.warning(
                f"{Fore.RED}{msg.status.value} at {Fore.RED}{msg.time}, Consider taking some mid summer essentials with you !! Current Temperature is {Fore.RED}{temperature}°C{Style.RESET_ALL}.")

        else:
            ctx.logger.info(
                f"{Fore.GREEN}{msg.status.value} at {Fore.GREEN}{msg.time} Temperature is perfect for you, Just go freely !! Current Temperature is {Fore.GREEN}{msg.temp}°C{Style.RESET_ALL}.")
